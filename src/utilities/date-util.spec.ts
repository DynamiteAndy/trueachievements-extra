...
describe('isValid', () => {
  ...
  ]).test.concurrent('should validate that date $input is either valid or invalid', ({ input, expected }) => {
...
describe('isAfterNow', () => {
  ...
  ]).test.concurrent('should validate that now is after $input', ({ input, expected }) => {
