      ...
      games.map((game: HTMLAnchorElement) => ({
        ...
        task: async (): Promise<void> => {
          ...
          await allConcurrently(
            ...
            gameAchievements.map((gameAchievement: HTMLElement) => ({
              ...
              task: (): void => {
                ...
                const walkthroughAchievement = walkthroughAchievements.find(
                  (walkthroughAchievement) =>
                    walkthroughAchievement.innerText.toLowerCase() === achievementName.toLowerCase()
                );
                ...
                if (walkthroughAchievement) {
                  ...
                  link.href = AchievementsRegex.Test.achievementUrlWithGamerId(link.href)
                    ? new URL(link.href).pathname
                    : link.href;
                } else {
                  const clonedAchievementRow = (
                    parsedDocument.querySelector(
                      `#${Constants.Templates.StaffWalkthroughImprovements.ManageWalkthroughPage.achievementRow}`
                    ) as HTMLTemplateElement
                  ).content.firstElementChild.cloneNode(true);
                  const achievementRow = template(clonedAchievementRow as HTMLElement, {
                    element: gameAchievement
                  });
                  const link = achievementRow.querySelector('a') as HTMLAnchorElement;
                  achievementRow.querySelector('a').href = AchievementsRegex.Test.achievementUrlWithGamerId(link.href)
                    ? new URL(link.href).pathname
                    : link.href;
...
export const makeTableLinksClickable = async (): Promise<void> => {
  ...
  const walkthroughContainer = document.querySelector(
    `.${Constants.Styles.StaffWalkthroughImprovements.ManageWalkthroughPage.containerJs}`
  ) as HTMLElement;
  const walkthroughId = selectedWalkthrough.getAttribute('data-walkthrough-id');
  const walthroughPreviewResponse = await memoizeFetch(
