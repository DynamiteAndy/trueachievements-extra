...
const applyBody = async(): Promise<void> => {
  ...
  asideColumn.appendChild(parsedDocument.querySelector(`.${Constants.Styles.StaffWalkthroughImprovements.WalkthroughPreview.populateAsideContentJs}`));

  extensionBody = asideColumn.querySelector(`.${Constants.Styles.StaffWalkthroughImprovements.WalkthroughPreview.populateAsideContentJs}`);
...
const getAsideContent = async(walkthroughId: string): Promise<void> => {
  ...
  const manageWalkthroughResponse = await memoizeFetch(`https://www.trueachievements.com/staff/walkthrough/managewalkthrough.aspx?walkthroughid=${walkthroughId}`);
...
const getPages = (manageWalkthroughDocument: Document): AsideContentPreviewPage[] => {
  return ([...manageWalkthroughDocument.querySelectorAll('#scrolllstWalkthroughPages tbody .c2 a')] as HTMLAnchorElement[]).map((page, index) => ({
...
const getPagesInWalkthrough = (walkthroughId: string, manageWalkthroughDocument: Document,
  featureDocument: Document, templateDocument: Document): HTMLElement => {
  if (!StaffRegex.Walkthroughs.Test.walkthroughPagePreviewUrlWithPageId()) {
    return null;
  }

  const pagesInWalkthroughSection = featureDocument.querySelector(`.${Constants.Styles.StaffWalkthroughImprovements.WalkthroughPreview.populateAsideContentWalkthroughPagesJs}`);
  const pagesinWalkthroughTable = pagesInWalkthroughSection.querySelector('tbody');
  const clonedSummaryRow = (templateDocument.querySelector(`#${Constants.Templates.StaffWalkthroughImprovements.WalkthroughPreview.walkthroughPagesSummary}`) as HTMLTemplateElement).content.firstElementChild.cloneNode(true);
  
  const summaryRow = template(clonedSummaryRow as HTMLElement, { urls: {
  ...
  getPages(manageWalkthroughDocument).forEach(populateAsideContentPreviewPage => {
    ...
    const pageRow = (pageId === populateAsideContentPreviewPage.id
      ? templateDocument.querySelector(`#${Constants.Templates.StaffWalkthroughImprovements.WalkthroughPreview.walkthroughPagesNumberedSelected}`) as HTMLTemplateElement
      : templateDocument.querySelector(`#${Constants.Templates.StaffWalkthroughImprovements.WalkthroughPreview.walkthroughPagesNumbered}`) as HTMLTemplateElement).content.firstElementChild.cloneNode(true);
    const templatedRow = template(pageRow as HTMLElement, { populateAsideContentPreviewPage, urls: {
...
const getAchievementsInWalkthrough = (manageWalkthroughDocument: Document,
  gameDocument: Document, featureDocument: Document, templateDocument: Document): HTMLElement => {
  const achievementsInWalkthroughSection = featureDocument.querySelector(`.${Constants.Styles.StaffWalkthroughImprovements.WalkthroughPreview.populateAsideContentWalkthroughAchievementsJs}`);
  const achievementsInWalkthroughTable = achievementsInWalkthroughSection.querySelector('ul');
  const walkthroughAchievements = ([...manageWalkthroughDocument.querySelectorAll('#scrolllstWalkthroughAchievementID tbody tr')] as HTMLElement[]);
  ...
  gameAchievements.forEach(gameAchievement => {
    ...
    const walkthroughAchievement = walkthroughAchievements.find(walkthroughAchievement => (walkthroughAchievement.querySelector('.c1') as HTMLElement)
    ...
    const achievementInfo: AsideContentPreviewAchievement = {
      ...
      page: pages.find(page => (walkthroughAchievement.querySelector('.c2') as HTMLElement).innerText.trim() === page.index.toString()),
  ...
  achievementsMerged.forEach(populateAsideContentPreviewAchievement => {
    const achievementRow = (templateDocument.querySelector(`#${Constants.Templates.StaffWalkthroughImprovements.WalkthroughPreview.walkthroughAchievements}`) as HTMLTemplateElement)
      .content.firstElementChild.cloneNode(true);

    const templatedRow = template(achievementRow as HTMLElement, { populateAsideContentPreviewAchievement, urls: {
...
const getWalkthroughThanks = async (walkthroughId: string, manageWalkthroughDocument: Document, featureDocument: Document): Promise<HTMLElement> => {
  ...
  return template(thanks as HTMLElement, { populateAsideContentPreviewThanks: { thread: threadUrl, total: '0' }});
};
