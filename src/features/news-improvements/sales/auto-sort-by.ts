...
const applyBody = async(): Promise<void> => {
  ...
  await allConcurrently('AutoSortBy - Tables', saleTables.map((saleTable: HTMLElement) => ({
    name: 'auto-sort-by-table',
    task: async () => {
      const tableHeader = ([...saleTable.querySelectorAll('.headers [data-sort]')] as HTMLElement[])
        .find(th => newsImprovements.sales.autoSortByValue.includes(th.innerText.replace(' ', '-').toLowerCase().trim()));
...
export default async (): Promise<void> => {
  ...
  if (newsImprovements.sales.autoSortByValue.includes('product') && newsImprovements.sales.autoSortByOrder === 'asc') return;
