...
const createAchievementGroup = (header: HTMLElement): AchievementGroup => ({
  ...
  maxAchievements: (header.querySelector('[title="Maximum achievements"], [title="Maximum Achievements"]') as HTMLElement).innerText,
  achievements: []
});
...
const getBaseAchievementGroup = (el: HTMLElement): AchievementGroup[] => {
  ...
  for (let child = el.querySelector('.ach-panels') as HTMLElement; child; child = child.nextSibling as HTMLElement) {
    ...
    baseAchievementGroup.achievements = baseAchievementGroup.achievements.concat(([...child.querySelectorAll('li')] as HTMLElement[]));
  }
...
const getDLCAchievementGroups = async(el: HTMLElement): Promise<AchievementGroup[]> => {
  ...
  await allConcurrently('individualProgress - Groups', dlcAchievementHeaders.map((dlcHeader: HTMLElement) => ({
...
const getAchievementGroups = async(el: HTMLElement): Promise<AchievementGroup[]> => {
  const achievementGroups = await allConcurrently('Game Improvements Individual Progress', [
    { name: 'game-improvements-individual-progress-base', task: async() => getBaseAchievementGroup(el) },
    { name: 'game-improvements-individual-progress-dlc', task: async() => getDLCAchievementGroups(el) } ]);

  return achievementGroups.flat();
};

const setGroupProgress = async(groups: AchievementGroup[]): Promise<void> => {
  const achievementGroups = [...document.querySelectorAll('.pnl-hd.game:not(.gamer):not([data-gid]), .pnl-hd.dlc')] as HTMLElement[];

  await allConcurrently('individualProgress - Groups', achievementGroups.map((achievementGroup: HTMLElement) => ({
    ...
    task: async (): Promise<void> => {
      ...
      const maxAchievements = achievementGroup.querySelector('[title="Maximum achievements"], [title="Maximum Achievements"]') as HTMLElement;

      const wonAchievements = grouping.achievements.filter(achievement => achievement.classList.contains('w'));
      const wonTrueAchievementScore = wonAchievements.length !== toInt(grouping.maxAchievements)
        ? wonAchievements.reduce((totalTAScore, achievement) => {
          const TAScore = toInt(achievement.querySelector('[data-af]')?.getAttribute('data-af'));
          return totalTAScore + TAScore;
        }, 0).toString()
        : grouping.maxTAScore;

      const wonGamerscore = wonAchievements.length !== toInt(grouping.maxAchievements)
        ? wonAchievements.reduce((totalGamerscore, achievement) => {
      ...
      maxTAScore.innerHTML = maxTAScore.innerHTML.replace(grouping.maxTAScore, `${insertSeperator(wonTrueAchievementScore)}/${grouping.maxTAScore}`);
      maxGamerscore.innerHTML = maxGamerscore.innerHTML.replace(grouping.maxGamerScore, `${insertSeperator(wonGamerscore)}/${grouping.maxGamerScore}`);
      maxAchievements.innerHTML = maxAchievements.innerHTML.replace(grouping.maxAchievements, `${insertSeperator(wonAchievements.length)}/${grouping.maxAchievements}`);
    }
  })), 5);
};
