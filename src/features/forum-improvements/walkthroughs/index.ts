...
export default async(): Promise<void> => {
  ...
  if (!ForumRegex.Test.viewBoardUrlWithBoardId() && !ForumRegex.Test.viewThreadUrlWithThreadId()) return;
  if (ForumRegex.Test.viewBoardUrlWithBoardId() && params.get('messageboardid') !== messageBoardId) return;
  ...
  allConcurrently('Walkthrough Page', [ 
    { name: 'walkthroughs-show-owner-progress', task: showOwnerProgress }
   ]);
};
