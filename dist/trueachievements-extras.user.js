...
/***/ "./node_modules/ajax-interceptor/index.js":
...
// EXTERNAL MODULE: ./node_modules/ajax-interceptor/index.js
var ajax_interceptor = __webpack_require__("./node_modules/ajax-interceptor/index.js");
    ...
    if (containerTop > 0) {
        ...
        if (stickyNavBarEnabled) {
            ...
            if (!stickyNavBarElement.classList.contains(Constants.Styles.Animations.yShow)) {
                if (opts.isRelativeToParent)
                    topStylePx = stickyElement.offsetHeight;
...
;// CONCATENATED MODULE: ./src/features/staff-walkthrough-improvements/edit-walkthrough/tinymce-fullwidth-toolbar.ts



let tinymceContainer;
let tinymceToolbar;
const tinymce_fullwidth_toolbar_variableProperty = Constants.Styles.Variables.StaffWalkthroughImprovements.EditWalkthroughPage.stickyTinymceToolbarTop;
const tinymce_fullwidth_toolbar_listen = async () => {
    window.addEventListener('scroll', async () => await applyStickyElementStyle(tinymce_fullwidth_toolbar_variableProperty, tinymceToolbar, tinymceContainer, {
...
const fixTinymceFullWidthToolbar = async () => {
    ...
    if (!tinymceContainer || !tinymceToolbar)
        return;
    ...
    await applyStickyElementStyle(tinymce_fullwidth_toolbar_variableProperty, tinymceToolbar, tinymceContainer, { noTransitionStyle: true, isRelativeToParent: true });
    await tinymce_fullwidth_toolbar_listen();
};
/* harmony default export */ const tinymce_fullwidth_toolbar = ({ fixTinymceFullWidthToolbar });

;// CONCATENATED MODULE: ./src/features/staff-walkthrough-improvements/edit-walkthrough/tinymce-floating-menu.ts


let tinymce_floating_menu_tinymceToolbar;
const tinymce_floating_menu_variableProperty = Constants.Styles.Variables.StaffWalkthroughImprovements.EditWalkthroughPage.stickyTinymceToolbarFloatingMenu;
const setTopPosition = () => {
    const actualTopPosition = getElementCoordinates(tinymce_floating_menu_tinymceToolbar);
    document.documentElement.style.setProperty(tinymce_floating_menu_variableProperty, `${tinymce_floating_menu_tinymceToolbar.offsetHeight + actualTopPosition.top}px`);
};
const tinymce_floating_menu_listen = () => {
    window.addEventListener('scroll', setTopPosition);
};
const fixTinymceFloatingMenus = async () => {
    const tinymceContainer = await waitForElement('.mce-tinymce');
    tinymce_floating_menu_tinymceToolbar = await waitForElement('.mce-tinymce .mce-container-body .mce-toolbar-grp', tinymceContainer);
    if (!tinymceContainer || !tinymce_floating_menu_tinymceToolbar)
        return;
    setTopPosition();
    tinymce_floating_menu_listen();
};
/* harmony default export */ const tinymce_floating_menu = ({ fixTinymceFloatingMenus });
